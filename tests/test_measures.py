...
def test_create_measure_output(
    ...
    measure_output = measures_view._create_measure(create_requirement.id, measure_input)
...
def test_get_measure_output_with_jira_issue(
    ...
    measure_output = measures_view._get_measure(create_measure_with_jira_issue.id)
...
def test_get_measure_output_invalid_id(measures_view: MeasuresView):
    with pytest.raises(HTTPException) as excinfo:
