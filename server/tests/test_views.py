...
def test_list_tasks(client, credentials, measure_id):
    response = client.get(
        f'/api/measures/{measure_id}/tasks', auth=credentials)
    assert response.status_code == 200
    assert type(response.json()) == list

def test_create_task(client, credentials, measure_id):
    response = client.post(
        f'/api/measures/{measure_id}/tasks', json=dict(summary='A new task'), 
        auth=credentials)
    assert response.status_code == 201
    task = response.json()
...
@pytest.fixture
def task(client, credentials, measure_id):
    return test_create_task(client, credentials, measure_id)

@pytest.fixture
def task_id(task):
    return task['id']

def test_get_task(client, credentials, task, task_id):
    response = client.get(f'/api/tasks/{task_id}', auth=credentials)
    assert response.status_code == 200
    assert response.json() == task

def test_update_task(client, credentials, task, task_id):
    task['summary'] = 'An updated task'
    response = client.put(f'/api/tasks/{task_id}', json=task, auth=credentials)
    assert response.status_code == 200
    assert response.json() == task

def test_delete_task(client, credentials, task_id):
    response = client.delete(f'/api/tasks/{task_id}', auth=credentials)
    assert response.status_code == 204
    response = client.get(f'/api/tasks/{task_id}', auth=credentials)
    assert response.status_code == 404